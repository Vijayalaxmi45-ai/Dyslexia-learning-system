...
3. **Alternative:** Leave root as `.` and rely on the root `vercel.json` (`installCommand` / `buildCommand` / `outputDirectory` — no legacy `builds` block, which can break the deploy step on current Vercel).
